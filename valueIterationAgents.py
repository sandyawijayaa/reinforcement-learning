...
class ValueIterationAgent(ValueEstimationAgent):
                ...
                if self.mdp.isTerminal(state):
                    ...
                    for action in actions:
                        ...
                        for pos, prob in transitions:
                            reward = self.mdp.getReward(state, action, pos)
                            total += prob * (reward + (self.discount * self.getValue(pos)))
                        scores.append(total)
                    if scores:
                        val = max(scores)
                        newValues[state] = val
    ...
    def computeQValueFromValues(self, state, action):
        ...
        #     q_val += prob * (reward + (discount_factor * next_value))
        # return q_val
    ...
    def computeActionFromValues(self, state):
        ...
        best_reward = 0.0
        for action in actions: 
            # calculate reward at each action
            action_reward = 0.0
            for next_state, prob in self.mdp.getTransitionStatesAndProbs(state, action):
                reward = self.mdp.getReward(state, action, next_state)
                discount_factor = self.discount
                next_value = self.getValue(next_state)
                action_reward += prob * (reward + (discount_factor * next_value))
            # find max reward and action that led to that reward
